...
class Settings(BaseSettings):
    ...
    # --- Environment -------------------------------------------------------
    env_name: str = Field(
        default="development", json_schema_extra={"env_name": "ENV_NAME"}
    )
    pyenv_version: str = Field(
        default="3.11.4", json_schema_extra={"env_name": "PYENV_VERSION"}
    )
    app_debug: bool = Field(default=False, json_schema_extra={"env_name": "APP_DEBUG"})

    # --- Application -------------------------------------------------------
    app_name: str = Field(
        default="Fast API application", json_schema_extra={"env_name": "APP_NAME"}
    )
    app_host: str = Field(
        default="localhost", json_schema_extra={"env_name": "APP_HOST"}
    )
    app_port: int = Field(default=1337, json_schema_extra={"env_name": "APP_PORT"})
    app_reload: bool = Field(default=True, json_schema_extra={"env_name": "APP_RELOAD"})
    ...
    app_jwt_algorithm: str = Field(
        default="HS256", json_schema_extra={"env_name": "APP_JWT_ALGORITHM"}
    )
    app_jwt_secret_key: str = Field(
        default="a_secret_key", json_schema_extra={"env_name": "APP_JWT_SECRET_KEY"}
    )
    app_jwt_expiration: int = Field(
        default=1200, json_schema_extra={"env_name": "APP_JWT_EXPIRATION"}
    )
    app_jwt_token_type: str = Field(
        default="bearer", json_schema_extra={"env_name": "APP_JWT_TOKEN_TYPE"}
    )
    app_health_check_api_key: str | None = Field(
        default=None, json_schema_extra={"env_name": "APP_HEALTH_CHECK_API_KEY"}
    )

    # --- Docker settings ----------------------------------------------------
    compose_bake: bool = Field(
        default="True", json_schema_extra={"env_name": "COMPOSE_BAKE"}
    )

    # --- Console Logger settings --------------------------------------------
    console_logger_level: str = Field(
        default="DEBUG", json_schema_extra={"env_name": "CONSOLE_LOGGER_LEVEL"}
    )
    ...
    console_logger_propagate: bool = Field(
        default=False, json_schema_extra={"env_name": "CONSOLE_LOGGER_PROPAGATE"}
    )

    # --- File Logger settings -----------------------------------------------
    file_logger_level: str = Field(
        default="DEBUG", json_schema_extra={"env_name": "FILE_LOGGER_LEVEL"}
    )
    file_logger_file_name: str = Field(
        default="fastapi_log", json_schema_extra={"env_name": "FILE_LOGGER_FILE_NAME"}
    )
    file_logger_file_size: int = Field(
        default=1000000, json_schema_extra={"env_name": "FILE_LOGGER_FILE_SIZE"}
    )
    file_logger_file_count: int = Field(
        default=3, json_schema_extra={"env_name": "FILE_LOGGER_FILE_COUNT"}
    )
    file_logger_dir: str = Field(
        default="./../logs", json_schema_extra={"env_name": "FILE_LOGGER_DIR"}
    )
    ...
    file_logger_mode: str = Field(
        default="w", json_schema_extra={"env_name": "FILE_LOGGER_MODE"}
    )
    file_logger_propagate: bool = Field(
        default=False, json_schema_extra={"env_name": "FILE_LOGGER_PROPAGATE"}
    )
    ...
    pg_db_name: str = Field(
        default="the_db_name", json_schema_extra={"env_name": "PG_DB_NAME"}
    )
    pg_db_host: str = Field(
        default="the_db_host", json_schema_extra={"env_name": "PG_DB_HOST"}
    )
    pg_db_port: int = Field(default=5432, json_schema_extra={"env_name": "PG_DB_PORT"})
    ...
    pg_db_backup_dir: str = Field(
        default="./src/db/db_scripts",
        json_schema_extra={"env_name": "PG_DB_BACKUP_DIR"},
    )
    ...
    pg_db_connection_pool_size: int = Field(
        default=100, json_schema_extra={"env_name": "PG_DB_CONNECTION_POOL_SIZE"}
    )
    pg_db_max_overflow: int = Field(
        default=0, json_schema_extra={"env_name": "PG_DB_MAX_OVERFLOW"}
    )
    pg_db_echo: bool = Field(
        default=False, json_schema_extra={"env_name": "PG_DB_ECHO"}
    )
    pg_db_future: bool = Field(
        default=True, json_schema_extra={"env_name": "PG_DB_FUTURE"}
    )
    pg_db_auto_commit: bool = Field(
        default=False, json_schema_extra={"env_name": "PG_DB_AUTO_COMMIT"}
    )
    pg_db_auto_flush: bool = Field(
        default=False, json_schema_extra={"env_name": "PG_DB_AUTO_FLUSH"}
    )
    pg_db_pre_ping: bool = Field(
        default=True, json_schema_extra={"env_name": "PG_DB_PRE_PING"}
    )
    pg_db_expire_on_commit: bool = Field(
        default=False, json_schema_extra={"env_name": "PG_DB_EXPIRE_ON_COMMIT"}
    )
    ...
    pg_local_user_id: int = Field(
        default=1000, json_schema_extra={"env_name": "PG_LOCAL_USER_ID"}
    )
    pg_local_group_id: int = Field(
        default=1000, json_schema_extra={"env_name": "PG_LOCAL_GROUP_ID"}
    )
    ...
    mongo_db_name: str = Field(
        default="the_mongo_db_name", json_schema_extra={"env_name": "MONGO_DB_NAME"}
    )
    mongo_db_host: str = Field(
        default="the_mongo_db_host", json_schema_extra={"env_name": "MONGO_DB_HOST"}
    )
    mongo_db_port: int = Field(
        default=27017, json_schema_extra={"env_name": "MONGO_DB_PORT"}
    )
    ...
    mongo_db_docker_host_volume_path: str = Field(
        default="~/data/db",
        json_schema_extra={"env_name": "MONGO_DB_DOCKER_HOST_VOLUME_PATH"},
    )
    ...
    nano_id_characters: str = Field(
        ...
        json_schema_extra={"env_name": "NANO_ID_CHARACTERS"},
    )
    nano_id_size: int = Field(
        default=25, json_schema_extra={"env_name": "NANO_ID_SIZE"}
    )
